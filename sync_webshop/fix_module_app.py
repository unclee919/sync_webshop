...
def run():
    ...
    # (The previous script output showed they went to frappe/sync_webshop/...)
    ...
    # 3. Double check all Webshop* DocTypes are in Sync Webshop module and standard
    doctypes = frappe.get_all("DocType", filters={"name": ["like", "Webshop%"]}, pluck="name")
    for name in doctypes:
        doc = frappe.get_doc("DocType", name)
        if doc.module != "Sync Webshop" or doc.custom != 0:
            print(f"Fixing DocType {name}...")
            doc.module = "Sync Webshop"
            doc.custom = 0
            doc.save(ignore_permissions=True)
            
            # Re-export to correct app
