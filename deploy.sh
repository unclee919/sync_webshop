...
deploy_frontend() {
  ...
  remote "set -eu; tar -xzf '$work/release.tgz' -C '$work'; test -f '$work/dist/index.html'; mkdir -p /var/backups; mv '$FRONTEND_DIR' '$backup'; mv '$work/dist' '$FRONTEND_DIR'; chown -R www-data:www-data '$FRONTEND_DIR'; if ! nginx -t; then mv '$FRONTEND_DIR' '${backup}.failed'; mv '$backup' '$FRONTEND_DIR'; chown -R www-data:www-data '$FRONTEND_DIR'; exit 1; fi; systemctl reload nginx; sleep 3; for route in / /products /cart /checkout /dashboard; do code=\$(curl -sS --max-time 20 -o /dev/null -w '%{http_code}' http://127.0.0.1\$route); test \"\$code\" = 200; done; mkdir -p '$(dirname "$APPROVED_REVISION_FILE")'; printf '%s\n' '$FRONTEND_REVISION' > '$APPROVED_REVISION_FILE'; rm -r '$work'; echo frontend_backup='$backup'; echo frontend_revision='$FRONTEND_REVISION'; echo frontend_files=\$(find '$FRONTEND_DIR' -type f | wc -l)"
